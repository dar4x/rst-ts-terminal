...
export const fetchServiceTypes = createAsyncThunk<
  ...
  void,
  { rejectValue: string }
>('branches/fetchServiceTypes', async (_, { rejectWithValue }) => {
  try {
...
export const fetchBranchesQueue = createAsyncThunk<branchesQueueI[], string>(
  'branches/queue',
  async (serviceID) => {
    try {
      const response = await axios.get<branchesQueueI[]>(
        `${BASE_URL}/branch/1/${serviceID}/service_queues/`
      );
      return response.data;
    } catch (error) {
      console.log(error);
